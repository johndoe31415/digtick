...
class ValueTable():
	...
	@classmethod
	def parse_from_file(self, f: "_io.TextIOWrapper"):
		pass
	...
	def at(self, input_var_dict: dict) -> int | None:
		index = 0
		for (i, varname) in enumerate(self.input_variable_names):
			bitno = self.input_variable_count - 1 - i
			if input_var_dict[varname]:
				index += (1 << bitno)
		return self._output_values[index]

	def print_kv(self, variable_order: list[str] | None = None, x_offset: int = 0, y_offset: int = 0, x_invert: bool = False, y_invert: bool = False):
