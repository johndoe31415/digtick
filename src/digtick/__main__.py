...
from .ActionMutateCircuit import ActionMutateCircuit
...
def main():
	...
	def genparser(parser):
		...
		parser.add_argument("-f", "--tbl-format", choices = [ "text", "pretty", "tex-horizontal", "tex-vertical", "typst-horizontal", "typst-vertical", "compact", "logisim" ], default = "text", help = "Print the table in the desired format. Can be one of %(choices)s, defaults to %(default)s.")
		parser.add_argument("-v", "--verbose", action = "count", default = 0, help = "Increase verbosity. Can be given multiple times.")
	...
	def genparser(parser):
		parser.add_argument("-I", "--iteration-count", metavar = "count", type = int, default = 1, help = "Run all transformations this amount of times. Defaults to %(default)d. Useful if you want, e.g., generate multiple variations of a shuffled expression.")
